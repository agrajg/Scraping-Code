...
with open('pin_data_try1.csv', 'w') as csvfile:
    fieldnames = ['attribution', 'url', 'first_name', 'last_name', 'id', 'note', 'color', 'link',
                  'url', 'count_following', 'count_followers', 'count_boards', 'count_likes',
                  'image_url', 'image_width', 'image_height']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    for uname in pin_list:
        # Calling search function
        search_pin(uname, token_ex)
