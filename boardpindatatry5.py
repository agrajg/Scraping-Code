...
import json
import re
# board_list = ["sharon6014/madewell-outfitting", "lolliki/italy"]
board_list = ["zaraofficial/spring-summer-2017-campaign" , "zaraofficial/woman-trf", "zaraofficial/man", "zaraofficial/kids","zaraofficial/zaradaily" ]
# board_list = [ "zaraofficial/man"]
example_board = "laurenkc17/weddings"
token1 = "AXE3eFg3YRL1p-3XKpPMcpGQuGKnFKhBNBPS8mxDfVOF9MBAhQAAAAA"
token2 = "ASBpyhNIEzblclnzPmaRoKpk"
token3 = "ATv9wCxMwF_4L9mz5N27FnBf3NIeFMRyhxWMhq1DfVOF9MBAhQAAAAA"
token4 = "AeRvQ_sqKyeQA1ePdtytAeRUZ6DgFMRyjwFLVqVDfVOF9MBAhQAAAAA"
token = token4
...
with open('boardpindata.csv', 'w', newline='') as csvfile:
    ...
    for board_id in board_list:
        ...
        while board_pins_url is not None:
            print("URL = ", board_pins_url)
            board_pins_json_obj = urllib.request.urlopen(board_pins_url)
            print ("------------------------------------------------------------------------------------------------------")
            board_pins_page_data = json.load(board_pins_json_obj)
            # Extracts elements of the board - convert into a function
            # call the function every time next page data in not none
            # or define a while loop before with condition that next page data is not NONE
            for item1 in board_pins_page_data['data']:
                ...
                print("URL = ", board_url)
                board_json_obj = urllib.request.urlopen(board_url)
                print ("------------------------------------------------------------------------------------------------------")
                ...
                print('counts_pins', " = ", type(board_page_data['data']['counts']['pins']), " = ", board_page_data['data']['counts']['pins'] is not None, " = ", board_page_data['data']['counts']['pins'])
                if board_counts_pins is None:
                    board_counts_pins = " "

                board_counts_collaborators = board_page_data['data']['counts']['collaborators']
                print('counts_collaborators', " = ", type(board_page_data['data']['counts']['collaborators']), " = ", board_page_data['data']['counts']['collaborators'] is not None, " = ", board_page_data['data']['counts']['collaborators'])
                if board_counts_collaborators is None:
                    board_counts_collaborators = " "

                board_counts_followers = board_page_data['data']['counts']['followers']
                print('counts_followers', " = ", type(board_page_data['data']['counts']['followers']), " = ", board_page_data['data']['counts']['followers'] is not None, " = ", board_page_data['data']['counts']['followers'])
                ...
                print('image_60x60_url', " = ", type(board_page_data['data']['image']['60x60']['url']), " = ", board_page_data['data'][
                                                                                         'image']['60x60']['url'] is not None, " = ", board_page_data['data']['image']['60x60']['url'])
                ...
                print('creator_first_name', " = ", type(item1['creator']['first_name']), " = ", item1['creator'][
                                                                                      'first_name'] is not None, " = ", item1['creator']['first_name'])
                ...
                print('original_link', " = ", type(item1['original_link']), " = ", item1['original_link'] is not None, " = ", item1['original_link'])
                ...
                print('board_id', " = ", type(item1['board']['id']), " = ", item1['board']['id'] is not None, " = ", item1['board']['id'])
                if pin_board_id is None:
                    pin_board_id = " "

                pin_board_name = item1['board']['name']
                print('board_name', " = ", type(item1['board']['name']), " = ", item1['board']['name'] is not None, " = ", item1['board']['name'])
                ...
                print('image_original_width', " = ", type(item1['image']['original']['width']), " = ", item1['image']['original']['width'] is not None, " = ", item1['image']['original']['width'])
                if pin_image_original_width is None:
                    pin_image_original_width = " "

                pin_image_original_height = item1['image']['original']['height']
                print('image_original_height', " = ", type(item1['image']['original']['height']), " = ", item1['image']['original']['height'] is not None, " = ", item1['image']['original']['height'])
                ...
                print('counts_comments', " = ", type(item1['counts']['comments']), " = ", item1['counts'][
                                                                                        'comments'] is not None, " = ", item1['counts']['comments'])
                ...
                print('counts_repins', " = ", type(item1['counts']['repins']), " = ", item1['counts'][
                                                                                        'repins'] is not None, " = ", item1['counts']['repins'])
            ...
            board_pins_url = board_pins_page_data['page']['next']
